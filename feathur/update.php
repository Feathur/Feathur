...
if(!$sFindSetting = $database->CachedQuery('SELECT * FROM settings WHERE `setting_name` LIKE :Setting', array('Setting' => 'templates_cleaned')))
{
  ...
  $sAdd->execute();
  // Rename old templates table
  $sAdd = $database->prepare("RENAME TABLE `templates` TO `templates_old`;");
  $sAdd->execute();
  // Rename new templates table to current templates table
  $sAdd = $database->prepare("RENAME TABLE `new_templates` TO `templates`;");
  $sAdd->execute();
