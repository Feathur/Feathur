...
if ($sAction == 'addtemplate')
{
  if ((!empty($_GET['type'])) && (!empty($_GET['url'])))
  {
	...
	if (is_array($sAdd)) die(json_encode($sAdd));
  }
}
...
if ($sAction == 'removetemplate')
{
  if (!empty($_GET['id']))
  {
	...
	if (is_array($sRemove)) die(json_encode($sRemove));
  }
  $sJson = 1;
}
...
if ($sAction == 'updatetemplate')
{
  if ((!empty($_GET['name'])) && (is_numeric($_GET['id'])))
  {
	...
	die(json_encode(array('result' => 'Template name updated.', 'type' => 'success', 'json' => '1')));
	} else {
	  die(json_encode(array('result' => 'You must specify a name for the template.', 'type' => 'error', 'json' => '1')));
	}
}
...
if (empty($sType))
{
	$sContent .= Templater::AdvancedParse(
				   $sAdminTemplate->sValue.'/templates',
				   $locale->strings,
				   array('Virtualization' => 0)
				 );
} else {
  ...
  $sContent .= Templater::AdvancedParse(
				 $sAdminTemplate->sValue.'/templates',
				 $locale->strings,
				 array('Virtualization' => $sType, 'TemplateList' => $sTemplateList, 'Errors' => $sErrors)
			   );
}