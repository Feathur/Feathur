...
if ($sType == search)
{
  ...
  {
	$sSearchUser = preg_replace('/user=/', '', $sSearch);
  ...
  {
	$sSearchType = preg_replace('/type=/', '', $sSearch);
...
$sContent = Templater::AdvancedParse(
              $sAdminTemplate->sValue.'/list',
			  $locale->strings,
			  array(
			    'Result'		=> $sResult,
				'ServerCount'	=> $sServerCount,
				'VPSCount'		=> $sVPSCount,
