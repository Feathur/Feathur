...
if (isset($uAction))
{
  ...
  if(is_array($sResult)) die(json_encode($sResult));
}

if (!isset($uType))
{
  $sContent .= Templater::AdvancedParse(
				 $sAdminTemplate->sValue.'/ippools',
				 $locale->strings,
				 array()
			   );
} else {
  if (isset($uPool))
  {
	...
	$sContent .= Templater::AdvancedParse(
				   ...
				   array(
					 ...
					 'Pool' => $uPool,
					 'IPList' => $sData['IPList'],
					 'ServerList' => $sData['ServerList'],
					 'AvailableServers' => $sData['AvailableServers'],
	...
	$sContent .= Templater::AdvancedParse(
				   $sAdminTemplate->sValue.'/ippools',
				   $locale->strings,
				   array('Type' => $uType, 'BlockList' => $sBlockList)
				 );
  }
}