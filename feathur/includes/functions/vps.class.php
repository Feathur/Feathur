...
class VPS extends CPHPDatabaseRecordClass
{
  ...
  public static function list_ipspace($sVPS)
  {
    ...
    if ($sIPs = $database->CachedQuery("SELECT * FROM ipaddresses WHERE `vps_id` = :VPSId", array('VPSId' => $sVPS->sId)))
    {
      ...
      {
        $sPrimary = ($value['ip_address'] == $sVPS->sPrimaryIP ? 1 : 0);
        if (!empty($value['block_id']))
        {
          ...
          $sIPAddresses[] = array(
            ...
            'gateway' => $sBlock->sGateway,
            'netmask' => $sBlock->sNetmask
          );
        }
        else
        {
  ...
  public static function list_templates($sVPS)
  {
    ...
    if ($sList = $database->CachedQuery('SELECT * FROM templates WHERE `type` = :Type AND `disabled` = 0', array('Type' => $sVPS->sType)))
    {
      ...
      {
        $sPrimary = ($value['id'] == $sVPS->sTemplateId ? 1 : 0);
        $sTemplates[] = array(
          'id' => $value['id'],
          'name' => $value['name'],
          'primary' => $sPrimary
  ...
  public static function list_servers($sVPS)
  {
    ...
    {
      $sCurrent = ($value['id'] == $sVPS->sServerId ? 1 : 0);
      $sServers[] = array(
        'id' => $value['id'],
        'name' => $value['name'],
        'current' => $sCurrent
    ...
    if ($sUserVPS = $database->CachedQuery('SELECT * FROM vps WHERE `user_id` = :UserId', array('UserId' => $sVPS->sUserId)))
    {
      foreach ($sUserVPS->data as $value)
      {
        ...
        $sUserVPSList[] = array(
          ...
          'server' => $sThisServer->sName,
          'server_ip' => $sThisServer->sIPAddress,
          'this' => $sThis
        );
      }
    }
  ...
  public static function check_ipspace($sServer, $sMinimum)
  {
    ...
    if (!empty($sBlocks))
    {
      ...
      else
      {
        return $sArray = array(
          'json' => 1,
          'type' => 'caution',
          'result' => 'There are not enough IPs for this VPS!'
        );
      }
    }
    else
    {
      return $sArray = array(
        'json' => 1,
        'type' => 'caution',
        'result' => 'There are no blocks assigned to the selected server!'
      );
    }
  }
  ...
  public static function add_template($uName, $uURL, $uType)
  {
    ...
    catch (Exception $e)
    {
      return $sArray = array(
        'json' => 1,
        'type' => 'error',
        'result' => 'Template/ISO URL is invalid or down.'
      );
    }
      ...
      if ($sPathSearch = $database->CachedQuery("SELECT * FROM templates WHERE `path` = :Path", array('Path' => $sPath)))
      {
        if ($uType == 'openvz')
        {
          while ($sUnique == 0)
          {
              ...
              if (strpos($sPath, $sValue) !== false)
              {
                if (!empty($sEnd))
                {
                  return $sArray = array(
                    'json' => 1,
                    'type' => 'error',
                    'result' => 'Template/ISO URL is invalid.'
                  );
                }
                $sEnd  = $sValue;
            ...
            if (!$sPathSearch = $database->CachedQuery("SELECT * FROM templates WHERE `path` = :Path", array('Path' => $sPath))) $sUnique = 1;
          }
        }
  ...
  public static function remove_template($sLocalSSH, $uId)
  {
    ...
    if (is_numeric($uId))
    {
      ...
      return $sArray = array(
        'json' => 1,
        'type' => 'success',
        'result' => 'Template/ISO has been deleted.',
        'reload' => 1
      );
    }
    else
    {
      return $sArray = array(
        'json' => 1,
        'type' => 'error',
        'result' => 'There is no template matching that id.'
      );
    }
  }
} 
