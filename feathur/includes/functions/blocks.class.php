      ...
      if (empty($sType))
      {
        if ($sIPs = $database->CachedQuery('SELECT * FROM ipaddresses WHERE `block_id` = :BlockId', array('BlockId' => $sPool)))
        {
          foreach($sIPs->data as $sValue)
          {
            ...
            $sIPList[] = array(
              'id' => $sValue['id'],
              'ip' => $sValue['ip_address'],
              'Owner' => $sTempUser->sUsername,
              'OwnerId' => $sTempUser->sId
            );
            unset($sTempUser);
            unset($sVPS);
          }
  ...
  public static function create_pool($sRequested)
  {
	...
	// If Per VPS is not a number check to make sure that it is not larger than the netmask (EG: impossible).
	  ...
	  if ($sCustomPerUser > 60000)
        ...
        $sNewBlock->uPerUser = (ctype_digit($sCustomPerVPS) ? $sCustomPerVPS : $sPerVPS);
  ...
  public static function vps_ipv6_block($sVPS)
  {
    ...
    if ($sBlockLookup = $database->CachedQuery('SELECT * FROM `useripv6blocks` WHERE `vps_id` = :VPSId', array('VPSId' => $sVPS->sId)))
    {
      foreach($sBlockLookup->data as $sRow)
      {
        ...
        for($i = 1; $i <= $sOffest; $i++) { $sUserPrefix .= '0000:'; }

        $sSecondary = $sBlock->sSecondary.str_pad($sRow['user_block'], 4, '0', STR_PAD_LEFT);

        $sIsBlock = (ctype_digit($sBlock->sPerUser) ? 0 : 1);
	$sIPv6Blocks[] = array(
          'id' => $sRow['id'],
          'prefix' => $sBlock->sPrefix.$sSecondary,
          'size' => $sBlock->sPerUser,
          'is_block' => $sIsBlock,
          'user_current' => str_pad($sRow['current'], 4, '0', STR_PAD_LEFT)
        );
      }
      return $sIPv6Blocks;
    }
