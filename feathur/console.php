...
if($sAction == connect){
	if((!empty($_POST['hostname'])) && (!empty($_POST['port']))){
		$sView = Templater::AdvancedParse($sTemplate->sValue.'/console', $locale->strings, array("connect" => "1", "VPS" => array("data" => $sVPS->uData), "Hostname" => $_POST['hostname'], "Port" => $_POST['port']));
	} else { 
