...
function feathur_ConfigOptions()
{
  ...
  foreach($sServers as $value) { $sServerList .= $value["ip"].','; }
...
function feathur_CreateAccount($sData)
{
  ...
  if($sSetupVPS["type"] == 'success')
  {
	...
	$sCustomField = mysql_fetch_array(mysql_query("SELECT * FROM tblcustomfields WHERE relid='{$sData["pid"]}' AND fieldname='feathurvpsid'"));
